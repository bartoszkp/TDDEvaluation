...
namespace ExampleSignalClient
{
    public class Program
    {
        static void Main(string[] args)
        {
            ...
            var sig = client.Add(new Signal()
            {
                DataType = DataType.Double,
                Granularity = Granularity.Month,
                Path = new Signals.Path { Components = new[] { "example", "path" } },
            });

            client.SetData(sig.Id.Value, new Datum[] {
            ...
            var result = client.GetData(sig.Id.Value, new DateTime(2000, 1, 1), new DateTime(2000, 1, 1));
