...
namespace WebService.Tests
{
    namespace WebService.Tests
    {
        ...
        public class SignalsWebServiceTests
        {
            ...
            [TestMethod]
            public void FirstOrderMissingValuePolicyCorrectlyFillValueInMissingData()
            {
                var existingSignal = SignalWith(1, DataType.Double, Granularity.Month, Path.FromString("root/signal1"));
                ...
                SetupSignalAndDatumWithPolicyMock(existingSignal, existingDatum, new DateTime(2000, 1, 1), new DateTime(2000, 4, 1), new DataAccess.GenericInstantiations.FirstOrderMissingValuePolicyDouble());
                var result = signalsWebService.GetData(existingSignal.Id.Value, new DateTime(2000, 1, 1), new DateTime(2000, 4, 1));
                AssertDatum(result, filledDatum);
            }

            [TestMethod]
            public void FirstOrderMissingValuePolicyCorrectlyFillQualityInMissingData()
            {
                ...
                var existingDatum = new Dto.Datum[]
                {
                        ...
                        new Dto.Datum {Quality = Dto.Quality.Poor, Timestamp = new DateTime(2000, 1, 3),  Value = 1 }
                };
                var filledDatum = new Dto.Datum[]
                {
                        new Dto.Datum {Quality = Dto.Quality.Good, Timestamp = new DateTime(2000, 1, 1),  Value = 1 },
                        new Dto.Datum {Quality = Dto.Quality.Poor, Timestamp = new DateTime(2000, 1, 2),  Value = 1 },
                        new Dto.Datum {Quality = Dto.Quality.Poor, Timestamp = new DateTime(2000, 1, 3),  Value = 1 }
                };
                SetupSignalAndDatumWithPolicyMock(existingSignal, existingDatum, new DateTime(2000, 1, 1), new DateTime(2000, 1, 4), new DataAccess.GenericInstantiations.FirstOrderMissingValuePolicyInteger());
                var result = signalsWebService.GetData(existingSignal.Id.Value, new DateTime(2000, 1, 1), new DateTime(2000, 1, 4));
                AssertDatum(result, filledDatum);
            }
            ...
            [TestMethod]
            public void FirstOrderMissingValuePolicyFillStartingDataWhenOlderDataNonExist()
            {
                var existingSignal = SignalWith(1, DataType.Double, Granularity.Month, Path.FromString("root/signal1"));
            ...
            public void WhenSettinShadowPolicy_IfItCreatesADependencyCycle_ThrowsAnException()
            {
                ...
                var shadowPolicy1 = GetShadowPolicy(signal1, signal2);
                var shadowPolicy2 = GetShadowPolicy(signal2, signal3);
                var shadowPolicy3 = GetShadowPolicy(signal3, signal1);

                signalsRepositoryMock = new Mock<ISignalsRepository>();
                missingValuePolicyRepositoryMock = new Mock<IMissingValuePolicyRepository>();

                SetupSetShadowPolicy(signal1, shadowPolicy1);

                SetupGetShadowPolicy(signal2.Id.Value, shadowPolicy2);
                SetupGetShadowPolicy(signal3.Id.Value, shadowPolicy3);

                SetupGetById(signal1);
                SetupGetById(signal2);
                SetupGetById(signal3);     
            ...
            private void SetupGetById(Dto.Signal signal)
            {
                signalsRepositoryMock
                    ...
                    .Returns(signal.ToDomain<Domain.Signal>());
            }
