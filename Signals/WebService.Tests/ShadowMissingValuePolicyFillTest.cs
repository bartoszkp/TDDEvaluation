...
namespace WebService.Tests
{
    ...
    public class ShadowMissingValuePolicyFillTest
    {
        ...
        [TestMethod]
        public void GivenASignalAndDatum_Integer_ReturnDatumShadow()
        {
            var existingSignal = SignalWith(1, DataType.Integer, Granularity.Month, Path.FromString("root/signal1"));
            var shadowSignal = SignalWith(2, DataType.Integer, Granularity.Month, Path.FromString("root/signal1/shadow"));
            var existingDatum = new Dto.Datum[]

            {
                        new Dto.Datum {Quality = Dto.Quality.Good, Timestamp = new DateTime(2000, 1, 1),  Value = (int)1 },
                        new Dto.Datum {Quality = Dto.Quality.Good, Timestamp = new DateTime(2000, 5, 1),  Value = (int)3 },
                        new Dto.Datum {Quality = Dto.Quality.Fair, Timestamp = new DateTime(2000, 8, 1),  Value = (int)5 }
            };
            var filledDatum = new Dto.Datum[]
            {
                        new Dto.Datum {Quality = Dto.Quality.Good, Timestamp = new DateTime(2000, 5, 1),  Value = (int)3 },
                        new Dto.Datum {Quality = Dto.Quality.Fair, Timestamp = new DateTime(2000, 6, 1),  Value = (int)4 },
            ...
            existingDatumFirst.Add(new Datum<int>() { Quality = Quality.Good, Timestamp = new DateTime(2000, 5, 1), Value = 3 });
            existingDatumFirst.Add(new Datum<int>() { Quality = Quality.Fair, Timestamp = new DateTime(2000, 6, 1), Value = 4 });
            existingDatumFirst.Add(new Datum<int>() { Quality = Quality.None, Timestamp = new DateTime(2000, 7, 1), Value = 0 });

            signalsRepositoryMock = new Mock<ISignalsRepository>();
            GivenASignal(existingSignal);
            signalsDataRepositoryMock = new Mock<ISignalsDataRepository>();
            signalsDataRepositoryMock.Setup(sdrm => sdrm.GetData<int>(It.Is<Domain.Signal>(x => x.Id == 2), new DateTime(2000, 5, 1), new DateTime(2000, 8, 1))).Returns(existingDatumFirst);
            choiseDataType(existingSignal, existingDatum, new DateTime(2000, 5, 1), new DateTime(2000, 8, 1));
            missingValuePolicyRepositoryMock = new Mock<IMissingValuePolicyRepository>();
            missingValuePolicyRepositoryMock
                .Setup(mvprm => mvprm.Get(It.IsAny<Domain.Signal>()))
        ...
        [TestMethod]
        public void GivenASignalAndDatum_String_ReturnDatumShadow()
        {
            var existingSignal = SignalWith(1, DataType.String, Granularity.Month, Path.FromString("root/signal1"));
            var shadowSignal = SignalWith(2, DataType.String, Granularity.Month, Path.FromString("root/signal1/shadow"));
            var existingDatum = new Dto.Datum[]

            {
                        new Dto.Datum {Quality = Dto.Quality.Good, Timestamp = new DateTime(2000, 1, 1),  Value = (string)"1" },
                        new Dto.Datum {Quality = Dto.Quality.Good, Timestamp = new DateTime(2000, 5, 1),  Value = (string)"3" },
                        new Dto.Datum {Quality = Dto.Quality.Fair, Timestamp = new DateTime(2000, 8, 1),  Value = (string)"5" }
            };
            var filledDatum = new Dto.Datum[]
            {
                        new Dto.Datum {Quality = Dto.Quality.Good, Timestamp = new DateTime(2000, 5, 1),  Value = (string)"3" },
                        new Dto.Datum {Quality = Dto.Quality.Fair, Timestamp = new DateTime(2000, 6, 1),  Value = (string)"4" },
                        new Dto.Datum {Quality = Dto.Quality.None, Timestamp = new DateTime(2000, 7, 1),  Value = (string)"" },
            };

            List<Datum<string>> existingDatumFirst = new List<Datum<string>>();
            existingDatumFirst.Add(new Datum<string>() { Quality = Quality.Good, Timestamp = new DateTime(2000, 5, 1), Value = "3" });
            existingDatumFirst.Add(new Datum<string>() { Quality = Quality.Fair, Timestamp = new DateTime(2000, 6, 1), Value = "4" });
            existingDatumFirst.Add(new Datum<string>() { Quality = Quality.None, Timestamp = new DateTime(2000, 7, 1), Value = "" });

            signalsRepositoryMock = new Mock<ISignalsRepository>();
            GivenASignal(existingSignal);
            signalsDataRepositoryMock = new Mock<ISignalsDataRepository>();
            signalsDataRepositoryMock.Setup(sdrm => sdrm.GetData<string>(It.Is<Domain.Signal>(x => x.Id == 2), new DateTime(2000, 5, 1), new DateTime(2000, 8, 1))).Returns(existingDatumFirst);
            choiseDataType(existingSignal, existingDatum, new DateTime(2000, 5, 1), new DateTime(2000, 8, 1));
            missingValuePolicyRepositoryMock = new Mock<IMissingValuePolicyRepository>();
            missingValuePolicyRepositoryMock
                .Setup(mvprm => mvprm.Get(It.IsAny<Domain.Signal>()))
                .Returns(new DataAccess.GenericInstantiations.ShadowMissingValuePolicyString()
                {
                    ShadowSignal = shadowSignal
                });
            var signalsDomainService = new SignalsDomainService(
        ...
        [TestMethod]
        [ExpectedException(typeof(Domain.Exceptions.NotMatchingDataTypesException))]
        public void GivenADoubleSignalAndDecimalShadowSignal_WhenSettingMVP_ExceptionIsThrown()
        {
            signalsRepositoryMock = new Mock<ISignalsRepository>();
        ...
        [TestMethod]
        [ExpectedException(typeof(Domain.Exceptions.NotMatchingGranularitiesException))]
        public void GivenAMonthSignalAndDayShadowSignal_WhenSettingMVP_ExceptionIsThrown()
        {
            signalsRepositoryMock = new Mock<ISignalsRepository>();
        ...
        [TestMethod]
        [ExpectedException(typeof(DependencyCycleException))]
        public void GivenASignal_WhenTryingToSetDependencyCycle_ExeptionIsThrown()
        {
            ...
            missingValuePolicyRepositoryMock
                .Setup(mvprm => mvprm.Get(It.Is<Domain.Signal>(signal => 
                (
                    signal.Id == 1
                    && signal.DataType == DataType.Double
                    && signal.Granularity == Granularity.Second
                ))))
                .Returns(new DataAccess.GenericInstantiations.ShadowMissingValuePolicyDouble() { ShadowSignal = signal1, Signal = signal2 });
            ...
            missingValuePolicyRepositoryMock
                ...
                .Returns(new DataAccess.GenericInstantiations.ShadowMissingValuePolicyDouble() { ShadowSignal = signal2, Signal = signal1 });

            var signalsDomainService = new SignalsDomainService(signalsRepositoryMock.Object, null, missingValuePolicyRepositoryMock.Object);

            signalsWebService = new SignalsWebService(signalsDomainService);

            signalsWebService.SetMissingValuePolicy(2, new Dto.MissingValuePolicy.ShadowMissingValuePolicy() { DataType = Dto.DataType.Double, ShadowSignal = signal1.ToDto<Dto.Signal>() });
        }
